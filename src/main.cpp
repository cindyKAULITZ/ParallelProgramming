...
void runKnn(char *trainFile, char *testFile, int k, int cuda) {
...
void findBestK(char *trainFile, int cuda) {
	cout << "Reading train" <<endl;
...
int main(int argc, char **argv)
{
	...
	if (strcmp(argv[1], "run") == 0) {
		runKnn(argv[2], argv[3], atoi(argv[4]), atoi(argv[5]));
	}
	else if (strcmp(argv[1], "findbest") == 0) {
		findBestK(argv[2], atoi(argv[3]));
	}
	else
		printUsageAndExit(argv);
}
